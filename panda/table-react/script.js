// Сначала хранится JSON с данными, это болванка созданная при помощи сервиса Mockaroo
// основной код начинается на 1405 строке.
const mainData = [
    {
        id: 1,
...
// Это наш главный компонет и в нем мы собираем наше СПА
class SearchApp extends React.Component {
    ...
    handleChange(event) {
        ...
        // При изменении в текстовом поле мы апдейтим стейт - реакт перерисовывает интерфейс
        this.setState({ search: searchValue, currentPage: 1 });
    }

    // метод для установки количества строк на странице. Мы передадим этот метод дочерним компонентам.
    setPostsPerPage(event) {
        this.setState({ postsPerPage: event.target.value, currentPage: 1 });
    }

    // метод для установки номера текущей страницы, для пагинации.
    setCurrentPage(event) {
        this.setState({ currentPage: event.number });
    }

    setSortingColumn(event) {
        console.log(event); // дебажил коечего забыл убрать
        this.setState({
            sortingColumn: event,
    ...
    render() {
        // создадим массив с данными клиентов из пропса и создадим переменную с поисковой строкой, обрежем ее от лишних символов и приведем в нижний регистр

        let clients = this.state.data;
        let searchString = this.state.search.trim().toLowerCase();
        // если в текстовом поле что-то есть то фильтруем массив с данными
        ...
        // определяем номер первого и последнего отображаемого элемента
        // делаем срез только этих элементов чтобы передать в рендер
        ...
        // посчитаем номера страниц и сохраним в массив для передачи элементу пагинации
        let pageNumbers = [];
        for (
            let i = 1;
        ...
        // запускам функцию рендер
        // передаем компонентам через пропсы все необходимые данные и методы
        ...
        return (
            <div>
                ...
                />
                <span className="found__entries">
                    Найдено записей: {clients.length}
                </span>
                <Table
                    data={currentPosts}
                    sortFunc={(e) => this.setSortingColumn(e)}
                />
            </div>
        );
    }
...
// компонент ТАБЛИЦА - отрисовывает шапку и потом циклом MAP отрисовывает строки таблицы из массива.
        ...
        return (
            <div>
                <table>
                    <tbody>
                        <tr>
                            <th onClick={() => this.props.sortFunc("id")}>
                                Номер
                            </th>
                            <th
                                onClick={() =>
                                    this.props.sortFunc("first_name")
                                }
                            >
                                Имя
                            </th>
                            <th
                                onClick={() => this.props.sortFunc("last_name")}
                            >
                                Фамилия
                            </th>
                            <th onClick={() => this.props.sortFunc("email")}>
                                Электронная почта
                            </th>
                            <th onClick={() => this.props.sortFunc("phone")}>
                                Телефон
                            </th>
                        </tr>
        ...
        return (
            <nav className="nav__bar">
                <div>
                    ...
                    <hr></hr>
                    <span>Страница: {this.props.currentPage}</span>
...
// Компонент СТРОКА_ТАБЛИЦЫ - рендерит одну строку таблицы, берет данные из пропсов
...
//Финальный аккорд  - рендерим наш Апп в ДОМ
